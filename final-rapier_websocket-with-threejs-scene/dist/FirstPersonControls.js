...
export class FirstPersonControls {
  constructor(scene, camera, renderer) {
    ...
    this.cameraHeight = 0; // 🌸 instead of 1.5
  ...
  computeCameraOrientation() {
    ...
    const distance = 500;
    this.camera.target.x = this.camera.position.x + distance * Math.sin(this.phi) * Math.cos(this.theta);
    this.camera.target.y = this.camera.position.y + distance * Math.cos(this.phi);
    this.camera.target.z = this.camera.position.z + distance * Math.sin(this.phi) * Math.sin(this.theta);

    // Set the camera's up vector to always point up (0,1,0)
    this.camera.up.set(0, 1, 0);

    // Look at the target while maintaining the up vector
